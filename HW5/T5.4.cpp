...
int main() {
    Address driverAddress("Bogdan", "Craiova");
    Driver driver("Matei", &driverAddress, "str.Libertatii");
    Bus bus("3B", &driverAddress, &driver);
    bus.addStop(new Address("Stop1", "Bechet"));
    bus.addStop(new Address("Stop2", "Dabuleni"));
    BusStation busStation(37);
    Car car("ToyotaPrius", &driverAddress, &driver);
    Apartment apartment(&driverAddress);
    Student student("Maria", &driverAddress);
    Application app;
    app.startBusEngine(&bus);
    Bus bus2("E1T", &driverAddress, &driver);
    ...
    if (isInBus) {
        cout << "Maria is in the bus driven by Matei." << endl;
    }
    else {
        cout << "Maria is not in the bus driven by Matei." << endl;
    }
